...
export const fetchUSGSEarthquakes = async (stateCode: string): Promise<Threat[]> => {
  try {
    ...
    // Filter by state using bounding boxes
    const stateBounds = getStateBounds(stateCode)

    const filtered = stateBounds
    ...
    return filtered
      .slice(0, 5) // limit to 5 earthquakes max
      .map((feature: any): Threat => {
        const p = feature.properties
        const magnitude: number = p.mag ?? 0
        const severity: SeverityLevel = magnitudeToSeverity(magnitude)

        return {
          id: feature.id ?? crypto.randomUUID(),
          source: 'USGS',
          type: `Earthquake M${magnitude.toFixed(1)}`,
          severity,
          headline: p.title ?? `M${magnitude.toFixed(1)} Earthquake`,
          description: buildEarthquakeDescription(p, magnitude),
          areaDesc: p.place ?? 'Unknown location',
          effective: p.time ?? Date.now(),
          expires: null,
          coordinates: [
...
const buildEarthquakeDescription = (p: any, magnitude: number): string => {
  const depth = p.dmin ? `Depth: ${p.dmin} km. ` : ''
  const felt = p.felt ? `Felt by ${p.felt} people. ` : ''
  const alert = p.alert ? `USGS alert level: ${p.alert}. ` : ''
  const tsunami = p.tsunami === 1 ? 'Tsunami warning issued. ' : ''

  return (
    `A magnitude ${magnitude.toFixed(1)} earthquake occurred ${p.place ?? 'in the region'}. ` +
    depth +
    felt +
    alert +
    tsunami +
...
// Bounding boxes for US states [minLat, maxLat, minLng, maxLng]
...
const getStateBounds = (stateCode: string): StateBounds | null => {
  const bounds: Record<string, StateBounds> = {
    AL: { minLat: 30.1, maxLat: 35.0, minLng: -88.5, maxLng: -84.9 },
    AK: { minLat: 51.2, maxLat: 71.4, minLng: -179.9, maxLng: -129.9 },
    ...
    FL: { minLat: 24.4, maxLat: 31.0, minLng: -87.6, maxLng: -79.9 },
    ...
    MD: { minLat: 37.9, maxLat: 39.7, minLng: -79.5, maxLng: -74.9 },
    ...
    WI: { minLat: 42.5, maxLat: 47.3, minLng: -92.9, maxLng: -86.2 },
    WY: { minLat: 40.9, maxLat: 45.0, minLng: -111.1, maxLng: -104.0 },
  }
  return bounds[stateCode] ?? null
}